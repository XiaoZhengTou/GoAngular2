...
export class AppComponent {
  ...
  ngOnInit() {
    this.globalClickCallbackFn = this.renderer.listen(this.elementRef.nativeElement, 'click', (event: any) => {
      console.log('全局监听事件' + event);
    });
    this.currentUser = JSON.parse(localStorage.getItem('currentUser'));
    this.userLoginService.currentUser.merge(this.userRegisterService.currentUser)
      .subscribe(
        data => {
          ...
          // 如果是从/login这个URL进行的登录，跳转到首页，否则什么也不做
    ...
    console.log('检测到你的浏览器语言' + browserLang);
