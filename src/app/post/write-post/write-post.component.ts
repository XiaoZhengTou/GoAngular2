...
export class WritePostComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public fileInputChangeHandler(): void {
    const fileInput = <HTMLInputElement>document.getElementById('img_input');
    const inputValue = fileInput.value;
    if (!inputValue) {
      return;
    }
    // 提交隐藏的表单，上传文件
    const fileForm = <HTMLFormElement>document.getElementById('file_upload_form');
    fileForm.actioc = 'fileuploadurl';
